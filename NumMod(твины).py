...
# modded by @zeticsce
# meta developer: @trololo_1ы (успешно спизжено)
...
class NumMod(loader.Module):
    ...
    async def numfiltercmd(self, message):
        ...
        if not args:
            return await utils.answer(
                ...
                f"<b>Работает так:</b>\n"
                f"[фильтр] (еб|бей|кусь|кусай|уеби|заразить) (1-10) ((@id|user)|link(даже полный линк ид'а))\n"
                f"[фильтр] вак|хи[лльсяйинг]\n"
                f"[фильтр] жертвы\n"
                f"[фильтр] болезни\n"
                f"[фильтр] лаб[уа]\n"
        ...
        if len(args) == 1 and not reply and args[0] != '-t':
            ...
            if user_id in filter_and_users['users']:
                ...
                await utils.answer(message, f"✅ Ид <code>{user_id}</code> пошёл нахуй.")
            elif len(filter_and_users['users']) <= 999:
                filter_and_users['users'].append(user_id)
                await utils.answer(message, f"✅ Ид <code>{user_id}</code> добро пожаловать ебать.")
            else:
    ...
    async def watcher(self, message):
        ...
        if send_mesа := re.search(
                r"(?P<z>бей\s|еб\s|кусай\s|заразить\s|еби\s|уеби\s{,2}\s)(?P<lvl>[1-9]?[0]?\s)?(?P<link>@[0-9a-z_]+|(?:https?://)?t\.me/[0-9a-z_]+|tg://openmessage\?user_id=(?P<id>[0-9]+))",
                text):
        ...
        elif re.search(r"вак|хи[лльсяйинг]{,2}", text):
